# Hi 👋, I'm Tyrel

I'm a senior software & platform developer from Victoria BC Canada.

## 🔧 Recent personal projects
- [Lathe](https://github.com/tyrelh/lathe): Pi Agent based software factory.
- [Claude plan usage monitor](https://github.com/tyrelh/claude-usage): Simple bash script that displays your current Claude plan usage in a friendly graph that refreshes automatically.
- [GitHub contribution history painter](https://github.com/tyrelh/git-history-painter): Go TUI that lets you draw on your GitHub contribution graph
- [Wealthsimple personal finance tools](https://github.com/tyrelh/finance-tools): Go TUI & Python backend that interacts with Wealthsimple to do recurring personal finance tasks
- [Agent configs](https://github.com/tyrelh/agent-config): Centralized repo for Claude & Codex skills and configuration
- [Container orchestrator written in Go](https://github.com/tyrelh/cube-orchestrator): Project from following this book [Build an Orchestrator in Go](https://www.manning.com/books/build-an-orchestrator-in-go-from-scratch)

## 📓 Articles
- [Shipping ECS Fargate logs to Datadog with FireLens (and what broke)](https://superflux.dev/blog/shipping-ecs-fargate-logs-to-datadog-with-firelens-and-what-broke) Oct 20 2025
- [AWS Solutions Architect Associate Certification](https://superflux.dev/blog/aws-solutions-architect-associate-certification) Jun 20 2025
- [Markdown based code block syntax highlighting](https://superflux.dev/blog/markdown-code-block-syntax-highlighting) Dec 24 2024
- [Static Site Search Part 3 - Optimizing the Index](https://superflux.dev/blog/static-site-search-optimizing-the-index) Apr 18 2024
- [Static Site Search Part 2 - Search Component](https://superflux.dev/blog/static-site-search-search-component) Apr 10 2024
- [Static Site Search Part 1 - Preprocessing Articles](https://superflux.dev/blog/static-site-search-preprocessing-articles) Apr 3 2024
- [Next.js Tag Pages](https://superflux.dev/blog/nextjs-tag-pages) Mar 25 2024
- [Book Notes: DevOps Handbook](https://superflux.dev/blog/book-notes-devops-handbook) Aug 27 2023
- [How I use my Synology NAS as my personal cloud](https://superflux.dev/blog/synology-nas-how-i-use-it-as-my-personal-cloud) Aug 15 2023
- [Deploy a Next.js Static Site to GitHub Pages](https://superflux.dev/blog/github-pages-using-nextjs) Jul 25 2023
